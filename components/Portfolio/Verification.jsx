import { Box, Grid, GridItem, Heading, Link, Text } from "@chakra-ui/react"
import { verification } from "../../mock/verification"
import Image from 'next/image'

const Production = () => {
    return (
        <Box textColor={'white'} px={{ base: 8, md: 24 }}>
            <Heading
                as="h1"
                fontSize={{ base: '24px', md: '48px', }}
                lineHeight={'32.4px'}
                fontWeight={'800'}
                py={{ base: 0, md: 4 }}
                mt={{ base: 8, md: 0 }}
                textAlign={'center'}
            >
                Verification
            </Heading>
            <Text textAlign={'center'}> We have Verified multiple Twitter, Facebook, and Instagram Accounts </Text>
            {verification.map(product => (
                <Box key={product.id}>
                    <Grid templateColumns={{ md: 'repeat(2, 1fr)' }} mb={24} mt={{ base: 0, sm: 8 }}>
                        <GridItem
                            width={'100%'}
                            px={{ base: 8 }}
                            p={0}
                        >
                            <Heading
                                as="h5"
                                fontSize={{ base: '24px', md: '36px' }}
                                lineHeight={'32.4px'}
                                fontWeight={'500'}
                                py={4}
                                mt={{ base: 8, md: 0 }}
                            >
                                {product.businessName}
                            </Heading>

                            <Text
                                fontWeight={'400'}
                                fontSize={{ md: '16px' }}
                                lineHeight={'30px'}
                                width={{ md: '60ch' }}
                                mb={4}
                            >

                                Business Type: {product.businessType}
                            </Text>

                            <Text
                                fontWeight={'400'}
                                fontSize={{ md: '16px' }}
                                lineHeight={'30px'}
                                width={{ md: '60ch' }}
                                mb={4}
                            >

                                Followers: {product.followerCount}
                            </Text>

                            <Text
                                fontWeight={'400'}
                                fontSize={{ md: '16px' }}
                                lineHeight={'30px'}
                                width={{ md: '60ch' }}
                                mb={4}
                            >

                                For more info:
                                <Link href={product.link}> See More </Link>
                            </Text>


                        </GridItem>

                        <GridItem>
                            <Image
                                src={product.image}
                                placeholder="blur"
                                quality={100}
                                width={product.width}
                                height={product.height}
                                objectFit="contain"
                                blurDataURL={`data:application/xml;base64,`}
                                sizes="(max-width: 768px) 100vw,(max-width: 1200px) 50vw, 33vw"
                                style={{
                                    maxWidth: '100%',
                                    height: 'auto',
                                }}
                            />
                        </GridItem>
                    </Grid>
                </Box>
            ))}

        </Box>
    )
}

export default Production